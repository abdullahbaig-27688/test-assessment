...
module.exports = function (server) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("joinMatch", ({ roomId, username }) => {
            ...
            // If two players are present, start the match
            if (rooms[roomId].players.length === 2) {
                io.to(roomId).emit("matchStart", { message: "Match started!" });
            }
        });
        // Handle player move
        socket.on("playerMove", ({ roomId, move }) => {
            console.log(`Move received in ${roomId}:`, move);
            io.to(roomId).emit("updateBoard", { move }); // Broadcast move to other player
        });
